...
void	op_ldi(t_vm *v, t_list *process)
{
	...
	//ft_printf("i = %d\n", octal_shift(B_OCT, 2, 3) % MEM_SIZE);
}
...
void	op_lld(t_vm *v, t_list *process)
{
	...
	//ft_printf("i = %d\n", octal_shift(B_OCT, 4, arg_nb)) % MEM_SIZE);
	...
	if (check_arg(10, B_OCT, arg_nb))
	{
		while (arg_nb--)
		{
			if (((B_OCT >> ((arg_nb + 1) * 2)) & 0b11) == 1 &&
				(ARENA(PC + 2 + shift) > 16 || !ARENA(PC + 2 + shift)))
				break ;
			// arg_nb = 2 - это номер регистар из которогоо береться значение
