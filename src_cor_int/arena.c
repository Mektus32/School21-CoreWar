...
void	arena(t_cor *cor)
{
	...
	if (!(cor->code = (char *)ft_memalloc(sizeof(char) * MEM_SIZE)))
	    exit_print("Error allocate memory");
	if (!(cor->live = (t_live *)ft_memalloc(sizeof(t_live))))
	    exit_print("Error allocate memory");
	if (!(cor->colormap = (unsigned char *)ft_memalloc(MEM_SIZE)))
	    exit_print("Error allocate memory");
	if (!(cor->bold = (unsigned char *)ft_memalloc(MEM_SIZE)))
	    exit_print("Error allocate memory");
	i = 0;
	while (i < cor->n)
	{
